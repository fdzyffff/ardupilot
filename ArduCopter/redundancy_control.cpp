...
void Copter::update_redundancy_control()
{
    ...
    // 检查 flightmode 是否有效，如果为空则使用默认值
    uint8_t current_mode_number = (uint8_t)Mode::Number::STABILIZE;
    if (flightmode != nullptr) {
        current_mode_number = (uint8_t)flightmode->mode_number();
    }
...
// 读取FPGA发来的串口数据
void Copter::read_redundancy_data_from_FPGA()
{
...
// 处理解析完的FPGA数据帧
void Copter::process_redundancy_frame_from_FPGA()
{
    // 将数据缓冲区转换为结构体
...
// 读取FMUa发来的串口数据
void Copter::read_redundancy_data_from_FMUa()
{
...
// 处理FMUa数据帧
void Copter::process_redundancy_frame_from_FMUa()
{
...
// 读取FMUb发来的串口数据
void Copter::read_redundancy_data_from_FMUb()
{
...
// 处理FMUb数据帧
void Copter::process_redundancy_frame_from_FMUb()
{
...
// 余度健康度评价值计算函数
uint8_t Copter::evaluate_redundancy_health()
{
