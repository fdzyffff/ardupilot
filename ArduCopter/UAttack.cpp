...
void UAttack::update()
{
    ...
    if (_Target_ptr_loc != nullptr) {
        if (_Target_ptr_loc->use_external_loc.get() == 1) {
            if (copter.g2.follow.have_target()) {
                Location tmp_loc;
                Vector3f tmp_vel;
                if (copter.g2.follow.get_target_location_and_velocity(tmp_loc, tmp_vel)) {
    ...
    // push frame angle to mission port
    {
        ...
        copter.uart.set_target_angle(gimbal_yaw, gimbal_pitch);
        copter.uart.set_target_loc(tmp_loc);
    } 
...
void UAttack::update_vel_bf_info()
{
    ...
    if (copter.position_ok() && AP::ahrs().get_velocity_NED(tmp_vel)) {
