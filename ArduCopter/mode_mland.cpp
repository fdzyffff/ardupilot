...
bool ModeMLand::init(const bool ignore_checks)
{
    ...
    // re-use guided mode
    bool ret = ModeGuided::init(ignore_checks);
    if (!ret) {
        gcs().send_text(MAV_SEVERITY_WARNING, "Fail from MLand [1]");
        return false;
    }
    _stage = 1;
...
void ModeMLand::run()
{
    ...
    bool use_yaw = false;
    float yaw_cd = 0.0f;
    // float home_alt_diff = (float)copter.ahrs.get_home().alt * 0.01f;

    Vector3f guided_destination;
    Vector3f guided_velocity;

    if (g2.follow.have_target() && copter.position_ok()) {
        Location target_loc;  // vector to lead vehicle + offset
        Vector3f vel_of_target;  // velocity of lead vehicle
        g2.follow.get_target_location_and_velocity_ofs(target_loc, vel_of_target);
        int32_t t_alt = target_loc.alt;
        target_loc.change_alt_frame(Location::AltFrame::ABSOLUTE);
        target_loc.alt = t_alt;
        target_loc.change_alt_frame(copter.current_loc.get_alt_frame());
        Vector3f dist_vec_offs = copter.current_loc.get_distance_NED(target_loc);
        ...
        // gcs().send_text(MAV_SEVERITY_INFO, "[%f, %f, %f]", dist_vec_offs.x, dist_vec_offs.y, dist_vec_offs.z);
        ...
        desired_velocity_neu_cms.x = (vel_of_target.x * 100.0f) + (dist_vec_offs_neu.x* 100.0f * kp);
        desired_velocity_neu_cms.y = (vel_of_target.y * 100.0f) + (dist_vec_offs_neu.y* 100.0f * kp);
        desired_velocity_neu_cms.z = (-vel_of_target.z * 100.0f) + (dist_vec_offs_neu.z* 100.0f * kp);
        ...
        Vector2f desired_velocity_xy_cms(desired_velocity_neu_cms.x - (vel_of_target.x * 100.0f), desired_velocity_neu_cms.y - (vel_of_target.y * 100.0f));
        ...
        copter.avoid.get_max_speed(pos_control->get_pos_z_p().kP().get(), pos_control->get_max_accel_z_cmss() * 0.5f, fabsf(dist_vec_offs_neu.z), copter.G_Dt);

        desired_velocity_neu_cms.x = (vel_of_target.x * 100.0f) + desired_velocity_xy_cms.x;
        desired_velocity_neu_cms.y = (vel_of_target.y * 100.0f) + desired_velocity_xy_cms.y;

        // use k230 and QR code for final approach
        if (copter.uk230.is_valid()) {
            desired_velocity_neu_cms.x = (vel_of_target.x * 100.0f) + copter.uk230.get_target_ef_vel_x() * 100.f;
            desired_velocity_neu_cms.y = (vel_of_target.y * 100.0f) + copter.uk230.get_target_ef_vel_y() * 100.f;
        }
        ...
        if (_stage == 1) {
            ...
            desired_velocity_neu_cms.z = (constrain_float(-copter.g.land_speed.get(), -300.f, 30.f));
        }
        ...
        if (_stage == 1 && dist_vec_offs.xy().length_squared() < 1.0f && fabsf(dist_vec_offs.z) < 1.0f) {
            _stage = 2;
            gcs().send_text(MAV_SEVERITY_WARNING, "Set MLand [%d]", _stage);
        }
        ...
        switch (g2.follow.get_yaw_behave()) {
            case AP_Follow::YAW_BEHAVE_FACE_LEAD_VEHICLE: {
                if (dist_vec_offs.xy().length_squared() > 1.0) {
                    yaw_cd = get_bearing_cd(Vector2f{}, dist_vec_offs.xy());
                    use_yaw = true;
                }
                break;
            }
    ...
    if ((now - last_log_ms >= 100) || (last_log_ms == 0)) {
        log_request = true;
        last_log_ms = now;
        // gcs().send_text(MAV_SEVERITY_WARNING, "dist_vec [%f, %f, %f | %f]", dist_vec.x, dist_vec.y, dist_vec.z, home_alt_diff);
        // gcs().send_text(MAV_SEVERITY_WARNING, "dist_vec_offs [%f, %f, %f | %f]", dist_vec_offs.x, dist_vec_offs.y, dist_vec_offs.z, home_alt_diff);
    }
...
bool ModeMLand::is_landing() const
{
    return _stage == 2;
}

uint32_t ModeMLand::wp_distance() const
{
    return g2.follow.get_distance_to_target() * 100;
}

int32_t ModeMLand::wp_bearing() const
{
 ...
 */
bool ModeMLand::get_wp(Location &loc) const
{
