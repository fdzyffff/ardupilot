...
void Copter::userhook_SuperSlowLoop_setgpsorigin() {
    ...
    if (millis() - last_update > 10000 && mocap_stat.n_count > 1) {
        ...
        if (ahrs.set_origin(ekf_origin_new)) {
            ...
            gcs().send_text(MAV_SEVERITY_WARNING, "alt: %d", ekf_origin_new.alt);
            gcs().send_text(MAV_SEVERITY_WARNING, "lng: %d", ekf_origin_new.lng);
            gcs().send_text(MAV_SEVERITY_WARNING, "lat: %d", ekf_origin_new.lat);
