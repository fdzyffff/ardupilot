...
bool ModeAttackVel::init(bool ignore_checks)
{
    ...
    if (copter.mode_guided.init(false)) {
        if (copter.uattack.is_active()) {
            ...
            pos_control->set_max_speed_accel_xy(copter.uattack.max_vel_xy.get()*100.f, wp_nav->get_wp_acceleration());
            pos_control->set_correction_speed_accel_xy(copter.uattack.max_vel_xy.get()*100.f, wp_nav->get_wp_acceleration());

            // initialize vertical speeds and acceleration
            pos_control->set_max_speed_accel_z(copter.uattack.max_vel_z.get()*100.f, copter.uattack.max_vel_z.get()*100.f, wp_nav->get_accel_z());
            pos_control->set_correction_speed_accel_z(copter.uattack.max_vel_z.get()*100.f, copter.uattack.max_vel_z.get()*100.f, wp_nav->get_accel_z());
...
void ModeAttackVel::run()
{
   // gcs().send_text(MAV_SEVERITY_WARNING, "ModeAttackVel run");
    ...
    if (delta_t > 20 ) {
        ...
        yaw_cd = copter.uattack.get_ef_info().x*100.f;
        float pitch_cd = copter.uattack.get_ef_info().y*100.f;
        if (pitch_cd > -3500.f) {
            pitch_cd = 0.0f;
        }
        Matrix3f tmp_m;
        ...
        float target_vel_nolim = copter.uattack.attack_vel*100.f;

        float vel_z_norm = 1.0f;
        float vel_z_nolim = tmp_output.z * target_vel_nolim;
        float vel_z_max = MAX(copter.uattack.max_vel_z.get() * 100.f, 100.0f);
        if (vel_z_nolim > vel_z_max) {
            vel_z_norm = vel_z_max/vel_z_nolim;
        }

        float vel_xy_norm = 1.0f;
        float vel_xy_nolim = tmp_output.xy().length() * target_vel_nolim;
        float vel_xy_max = MAX(copter.uattack.max_vel_xy.get() * 100.f, 100.0f);
        if (vel_xy_nolim > vel_xy_max) {
            vel_xy_norm = vel_xy_max/vel_xy_nolim;
        }
        float target_vel = target_vel_nolim*MIN(vel_xy_norm, vel_z_norm);
