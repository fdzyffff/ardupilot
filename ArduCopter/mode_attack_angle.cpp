...
bool ModeAttack_angle::init(bool ignore_checks)
{
    ...
    if (copter.Ucam.is_active()) {
        ...
        copter.g2.user_parameters.Thr_pid.set_dt(1.0/copter.scheduler.get_loop_rate_hz());
        // copter.g2.user_parameters.Thr_pid.set_integrator(motors->get_throttle_hover());
        return true;
    }
    return false;
}
...
void ModeAttack_angle::run()
{
    ...
    float target_pitch_norm = -copter.g2.user_parameters.fly_attack_angle/2500.f; //-1 ~ +1
    float current_pitch_norm = (degrees(copter.ahrs_view->pitch)*100.f)/2500.f;; //-1 ~ +1
    float target_thr_before_boosted = motors->get_throttle_hover() + constrain_float(-copter.g2.user_parameters.Thr_pid.update_all(target_pitch_norm, current_pitch_norm), -1.0f, 1.0f);

    target_thr_before_boosted = constrain_float(target_thr_before_boosted, motors->get_throttle_hover()/2.0f, 0.6f);

    if (millis() - _last_print_ms > 500) {
        _last_print_ms = millis();
        gcs().send_text(MAV_SEVERITY_INFO, "Thr %f", target_thr_before_boosted);
    }
    attitude_control->set_throttle_out(target_thr_before_boosted,
                                       true,
                                       g.throttle_filt);
    // if (_stage == 1) {
