...
bool Copter::ModeTestStar::init(bool ignore_checks)
{
    if (copter.position_ok() || ignore_checks) {
        ...
        copter.init_TestStart();

        gcs().send_text(MAV_SEVERITY_INFO,"Start STAR Mode");
        return true;
    } else {
        return false;
    }
...
bool Copter::ModeTestStar::loiter_start()
{
...
void Copter::ModeTestStar::rtl_start()
{
    _mode = Auto_RTL;
...
void Copter::ModeTestStar::takeoff_start(const Location& dest_loc)
{
    _mode = Auto_TakeOff;
...
void Copter::ModeTestStar::wp_start(const Vector3f& destination)
{
    _mode = Auto_WP;
...
// auto_wp_start - initialises waypoint controller to implement flying to a particular destination
void Copter::ModeTestStar::wp_start(const Location_Class& dest_loc)
{
    _mode = Auto_WP;
...
void Copter::ModeTestStar::land_start()
{
...
// auto_land_start - initialises controller to implement a landing
void Copter::ModeTestStar::land_start(const Vector3f& destination)
{
    _mode = Auto_Land;
...
void Copter::ModeTestStar::circle_movetoedge_start(const Location_Class &circle_center, float radius_m)
{
...
void Copter::ModeTestStar::circle_start()
{
    _mode = Auto_Circle;
...
void Copter::ModeTestStar::spline_start(const Location_Class& destination, bool stopped_at_start,
...
void Copter::ModeTestStar::nav_guided_start()
{
...
bool Copter::ModeTestStar::landing_gear_should_be_deployed() const
{
    switch(_mode) {
    case Auto_Land:
        return true;
...
void Copter::ModeTestStar::payload_place_start()
{
    // set target to stopping point
    Vector3f stopping_point;
    loiter_nav->get_stopping_point_xy(stopping_point);
...
bool Copter::ModeTestStar::start_command(const AP_Mission::Mission_Command& cmd)
{
    TestStarState.current_cmd = cmd;
    switch(cmd.id) {
...
bool Copter::ModeTestStar::verify_command_callback(const AP_Mission::Mission_Command& cmd)
{
    if (copter.flightmode == &copter.mode_teststar) {
...
bool Copter::ModeTestStar::do_guided(const AP_Mission::Mission_Command& cmd)
{
...
uint32_t Copter::ModeTestStar::wp_distance() const
{
    return wp_nav->get_wp_distance_to_destination();
}

int32_t Copter::ModeTestStar::wp_bearing() const
{
    return wp_nav->get_wp_bearing_to_destination();
}

bool Copter::ModeTestStar::get_wp(Location_Class& destination)
{
    switch (_mode) {
    case Auto_NavGuided:
...
bool Copter::ModeTestStar::verify_command(const AP_Mission::Mission_Command& cmd)
{
    switch (cmd.id) {
    //
    // navigation commands
...
void Copter::ModeTestStar::takeoff_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeTestStar::wp_run()
{
    // if not auto armed or motor interlock not enabled set throttle to zero and exit immediately
    if (!motors->armed() || !ap.auto_armed || !motors->get_interlock()) {
...
//      called by auto_run at 100hz or more
void Copter::ModeTestStar::spline_run()
{
    // if not auto armed or motor interlock not enabled set throttle to zero and exit immediately
    if (!motors->armed() || !ap.auto_armed || !motors->get_interlock()) {
        // To-Do: reset waypoint origin to current location because copter is probably on the ground so we don't want it lurching left or right on take-off
...
//      called by auto_run at 100hz or more
void Copter::ModeTestStar::land_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeTestStar::rtl_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeTestStar::circle_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeTestStar::nav_guided_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeTestStar::loiter_run()
{
    // if not auto armed or motor interlock not enabled set throttle to zero and exit immediately
    if (!motors->armed() || !ap.auto_armed || ap.land_complete || !motors->get_interlock()) {
        zero_throttle_and_relax_ac();
...
void Copter::ModeTestStar::payload_place_start(const Vector3f& destination)
{
...
//      called by auto_run at 100hz or more
void Copter::ModeTestStar::payload_place_run()
{
...
bool Copter::ModeTestStar::payload_place_run_should_run()
{
    // muts be armed
    if (!motors->armed()) {
        return false;
...
void Copter::ModeTestStar::payload_place_run_loiter()
{
    // loiter...
    land_run_horizontal_control();
...
void Copter::ModeTestStar::payload_place_run_descend()
{
    land_run_horizontal_control();
    land_run_vertical_control();
}
...
Location_Class Copter::ModeTestStar::terrain_adjusted_location(const AP_Mission::Mission_Command& cmd) const
{
...
void Copter::ModeTestStar::do_takeoff(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    Location_Class target_loc(cmd.content.location);
    const Location_Class &current_loc = copter.current_loc;
...
void Copter::ModeTestStar::do_land(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_loiter_unlimited(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_circle(const AP_Mission::Mission_Command& cmd)
{
...
// note: caller should set yaw_mode
void Copter::ModeTestStar::do_loiter_time(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_nav_guided_enable(const AP_Mission::Mission_Command& cmd)
{
    if (cmd.p1 > 0) {
...
void Copter::ModeTestStar::do_guided_limits(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_nav_delay(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_wait_delay(const AP_Mission::Mission_Command& cmd)
{
    condition_start = millis();
    condition_value = cmd.content.delay.seconds * 1000;     // convert seconds to milliseconds
}

void Copter::ModeTestStar::do_within_distance(const AP_Mission::Mission_Command& cmd)
{
    condition_value  = cmd.content.distance.meters * 100;
}

void Copter::ModeTestStar::do_yaw(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_change_speed(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_set_home(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_roi(const AP_Mission::Mission_Command& cmd)
{
    auto_yaw.set_roi(cmd.content.location);
}

// point the camera to a specified angle
void Copter::ModeTestStar::do_mount_control(const AP_Mission::Mission_Command& cmd)
{
#if MOUNT == ENABLED
...
void Copter::ModeTestStar::do_digicam_configure(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_digicam_control(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_parachute(const AP_Mission::Mission_Command& cmd)
{
    switch (cmd.p1) {
...
void Copter::ModeTestStar::do_gripper(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeTestStar::do_winch(const AP_Mission::Mission_Command& cmd)
{
    // Note: we ignore the gripper num parameter because we only support one gripper
...
void Copter::ModeTestStar::do_payload_place(const AP_Mission::Mission_Command& cmd)
{
    // if location provided we fly to that location at current altitude
    if (cmd.content.location.lat != 0 || cmd.content.location.lng != 0) {
        // set state to fly to location
...
void Copter::ModeTestStar::do_RTL(void)
{
...
bool Copter::ModeTestStar::verify_takeoff()
{
...
bool Copter::ModeTestStar::verify_land()
{
    bool retval = false;
...
bool Copter::ModeTestStar::verify_payload_place()
{
...
bool Copter::ModeTestStar::verify_loiter_unlimited()
{
    return false;
}

// verify_loiter_time - check if we have loitered long enough
bool Copter::ModeTestStar::verify_loiter_time()
{
...
bool Copter::ModeTestStar::verify_RTL()
{
...
bool Copter::ModeTestStar::verify_wait_delay()
{
...
bool Copter::ModeTestStar::verify_within_distance()
{
...
bool Copter::ModeTestStar::verify_yaw()
{
...
bool Copter::ModeTestStar::verify_nav_wp(const AP_Mission::Mission_Command& cmd)
{
...
bool Copter::ModeTestStar::verify_circle(const AP_Mission::Mission_Command& cmd)
{
...
bool Copter::ModeTestStar::verify_spline_wp(const AP_Mission::Mission_Command& cmd)
{
    // check if we have reached the waypoint
    if ( !copter.wp_nav->reached_wp_destination() ) {
        return false;
...
bool Copter::ModeTestStar::verify_nav_guided_enable(const AP_Mission::Mission_Command& cmd)
{
...
bool Copter::ModeTestStar::verify_nav_delay(const AP_Mission::Mission_Command& cmd)
{
