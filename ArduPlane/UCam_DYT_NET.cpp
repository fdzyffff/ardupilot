...
UCam_DYT_NET::UCam_DYT_NET(UAttack &frotend_in, AP_HAL::UARTDriver* port_in):
    UCam_base(frotend_in)
{
    FD1_uart_ptr = new FD_DYT_NET(port_in);
...
void UCam_DYT_NET::update() {
    static uint32_t last_update_ms = millis();
    _yaw_rate_filter.apply(degrees(plane.ahrs.get_yaw_rate_earth()));

    FD1_uart_ptr->read();
    FD_DYT_NET_msg_DYTTELEM &tmp_msg = FD1_uart_ptr->get_msg_DYTTELEM();
...
// APM -> DYT
void UCam_DYT_NET::fill_state_msg()
{
    ...
    FD_DYT_NET_msg_APM2DYTTELEM &tmp_msg = FD1_uart_ptr->get_msg_APM2DYTTELEM();

    tmp_msg._msg_1.content.msg.header.head_1 = FD_DYT_NET_msg_APM2DYTTELEM::PREAMBLE1;
    tmp_msg._msg_1.content.msg.header.head_2 = FD_DYT_NET_msg_APM2DYTTELEM::PREAMBLE2;
...
void UCam_DYT_NET::do_cmd() {
    ;
}

bool UCam_DYT_NET::is_valid() {
    return _valid;
}

void UCam_DYT_NET::handle_info(float p1, float p2) {
        // if (!_valid) {
...
void UCam_DYT_NET::handle_info_test(float p1, float p2) {
    FD_DYT_NET_msg_DYTTELEM &tmp_msg = FD1_uart_ptr->get_msg_DYTTELEM();
