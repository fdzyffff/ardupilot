...
void ModeQMLand::update()
{
    ;
    // for now we don't need manual input when landing 
...
void ModeQMLand::run()
{
    ...
    Location target_loc = plane.ubase.get_target_loc();
    Vector3f target_vel = plane.ubase.get_target_vel();

    plane.set_velocity_match(target_vel.xy());
    plane.update_target_location(plane.next_WP_loc, target_loc);
