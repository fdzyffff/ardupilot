...
bool ModeLoiter_Target::_enter()
{
    ...
    if (plane.uattack.have_target_loc()) {
        plane.next_WP_loc.lat = plane.uattack.get_target_loc().lat;
        plane.next_WP_loc.lng = plane.uattack.get_target_loc().lng;
    } else {
        gcs().send_text(MAV_SEVERITY_INFO, "Tgt NOT");
    }
    return true;
}

void ModeLoiter_Target::update()
{
    plane.calc_nav_roll();
    if (plane.stick_mixing_enabled() && plane.flight_option_enabled(FlightOptions::ENABLE_LOITER_ALT_CONTROL)) {
...
bool ModeLoiter_Target::isHeadingLinedUp(const Location loiterCenterLoc, const Location targetLoc)
{
...
bool ModeLoiter_Target::isHeadingLinedUp_cd(const int32_t bearing_cd) {
...
bool ModeLoiter_Target::isHeadingLinedUp_cd(const int32_t bearing_cd, const int32_t heading_cd)
{
...
void ModeLoiter_Target::navigate()
{
...
void ModeLoiter_Target::update_target_altitude()
{
    if (plane.stick_mixing_enabled() && (plane.flight_option_enabled(FlightOptions::ENABLE_LOITER_ALT_CONTROL))) {
        return;
    }
