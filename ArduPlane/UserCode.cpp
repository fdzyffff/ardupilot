...
void Plane::userhook_1Hz()
{
    ...
    if ((g2.print.get() & (1<<0)) && uattack.display_info.new_data) { // 1
    ...
    if (g2.print.get() & (1<<4)) { // 16
        gcs().send_text(MAV_SEVERITY_WARNING, "rpyt (%0.1f , %0.1f , %0.1f , %0.2f)", uattack.get_target_roll_angle(), uattack.get_target_pitch_rate(), uattack.get_target_yaw_rate(), g2.attack_throttle.get());
    }
    // if (g2.print.get() & (1<<5)) { // 32
