...
void UK230::update_target_roll_rate() {
    ...
    //Limit roll rate
    float limit_roll_rate = plane.g2.user_rate_limit.get();
    _target_roll_rate = constrain_float(_target_roll_rate, -limit_roll_rate, limit_roll_rate);

    //Limit roll
    float current_roll = degrees(plane.ahrs.get_roll());
    float limit_roll = MAX(plane.g2.user_angle_limit.get(), 0.f);
...
void UK230::update_target_pitch_rate() {
    ...
    //Limit pitch rate
    float limit_pitch_rate = plane.g2.user_rate_limit.get();
    _target_pitch_rate = constrain_float(_target_pitch_rate, -limit_pitch_rate, limit_pitch_rate);

    //Limit pitch
    float current_pitch = degrees(plane.ahrs.get_pitch());
    float limit_pitch = MAX(plane.g2.user_angle_limit.get(), 0.f);
