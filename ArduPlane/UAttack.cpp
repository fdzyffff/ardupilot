...
void UAttack::UDelay::push()
{
    ...
    _buffer[_idx].roll = plane.ahrs.get_roll();
    _buffer[_idx].pitch = plane.ahrs.get_pitch();
    _buffer[_idx].yaw = plane.ahrs.get_yaw();
...
void UAttack::init()
{
    ...
    const float yaw_hz = filt_yaw_hz.get();
    const float pitch_hz = filt_pithc_hz.get();
    _yaw_sample_filter.set_cutoff_frequency(yaw_hz, 30.f);
    _pitch_sample_filter.set_cutoff_frequency(pitch_hz, 30.f);
    gcs().send_text(MAV_SEVERITY_WARNING, "Target FILT HZ [%0.0f, %0.0f]", yaw_hz, pitch_hz);
}
