...
#ifndef DEPTH_MAX_INSTANCES
#define DEPTH_MAX_INSTANCES 1
#endif
...
class AP_Depth
{
    friend class AP_Depth_Backend;

public:
    AP_Depth();

    /* Do not allow copies */
    AP_Depth(const AP_Depth &other) = delete;
    AP_Depth &operator=(const AP_Depth&) = delete;

    // get singleton
    static AP_Depth *get_singleton(void) {
        return _singleton;
    }
...
private:
    // singleton
    static AP_Depth *_singleton;
    
    // how many drivers do we have?
    uint8_t _num_drivers;
    AP_Depth_Backend *drivers[DEPTH_MAX_DRIVERS];
    ...
    bool _add_backend(AP_Depth_Backend *backend);
