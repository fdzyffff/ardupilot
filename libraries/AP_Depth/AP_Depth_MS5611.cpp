...
/*
  constructor
 */
AP_Depth_MS56XX::AP_Depth_MS56XX(AP_Depth &depth, AP_HAL::OwnPtr<AP_HAL::Device> dev, enum MS56XX_TYPE ms56xx_type)
    : AP_Depth_Backend(depth)
    , _dev(std::move(dev))
    , _ms56xx_type(ms56xx_type)
{
}

AP_Depth_Backend *AP_Depth_MS56XX::probe(AP_Depth &depth,
...
bool AP_Depth_MS56XX::_init()
{
    ...
    enum DevTypes devtype = DEVTYPE_BARO_MS5837;
    switch (_ms56xx_type) {
    case BARO_MS5837:
    ...
    if (_ms56xx_type == BARO_MS5837) {
        _frontend.set_type(_instance, AP_Depth::BARO_TYPE_WATER);
    }
    ...
    _dev->register_periodic_callback(10 * AP_USEC_PER_MSEC,
                                     FUNCTOR_BIND_MEMBER(&AP_Depth_MS56XX::_timer, void));
    return true;
}

uint16_t AP_Depth_MS56XX::_read_prom_word(uint8_t word)
{
...
uint32_t AP_Depth_MS56XX::_read_adc()
{
    uint8_t val[3];
...
bool AP_Depth_MS56XX::_read_prom_5637(uint16_t prom[8])
{
    /*
 ...
 * pressure.
*/
void AP_Depth_MS56XX::_timer(void)
{
    uint8_t next_cmd;
    uint8_t next_state;
...
void AP_Depth_MS56XX::_update_and_wrap_accumulator(uint32_t *accum, uint32_t val,
...
void AP_Depth_MS56XX::_calculate_5837()
{
    int32_t dT, TEMP;
    int64_t OFF, SENS;
