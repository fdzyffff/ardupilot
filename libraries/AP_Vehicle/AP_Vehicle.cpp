...
void AP_Vehicle::send_watchdog_reset_statustext()
{
    ...
    gcs().send_text(MAV_SEVERITY_CRITICAL,
                    ...
                    pd.scheduler_task,  //T
                    pd.semaphore_line,  //SL
                    pd.fault_line,      //FL
                    pd.fault_type,      //FT
                    (unsigned)pd.fault_addr,  //FA
                    pd.fault_thd_prio,        //FTP
                    (unsigned)pd.fault_lr,    //FLR
                    (unsigned)pd.fault_icsr,  //FICSR
                    pd.last_mavlink_msgid,    //MM
                    pd.last_mavlink_cmd,      //MC
                    (unsigned)pd.internal_errors,      //IE
                    (unsigned)pd.internal_error_count, //IEC
                    pd.thread_name4                    //TN
        );
}
