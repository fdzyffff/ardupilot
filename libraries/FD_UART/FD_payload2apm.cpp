...
FD_payload2apm::FD_payload2apm(void)
{
    _enable = false;
...
void FD_payload2apm::parse(uint8_t temp)
{
    switch (_msg.msg_state)
    {
        ...
        case FD_UART_msg_parser::FD_UART_DATA:
            ...
            {
                _msg.msg_state = FD_UART_msg_parser::FD_UART_SUM;
            }
            break;
        case FD_UART_msg_parser::FD_UART_SUM:
            _msg.data[_msg.read_idx] = temp;
            _msg.msg_state = FD_UART_msg_parser::FD_UART_PREAMBLE1;
...
void FD_payload2apm::process_message(void)
{
    int16_t i = 0;
