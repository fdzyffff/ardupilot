#include "FD1_msg_LS_status.h"
// #include <GCS_MAVLink/GCS.h>

FD1_msg_LS_status::FD1_msg_LS_status(void)
{
    _enable = false;
...
void FD1_msg_LS_status::parse(uint8_t temp)
{
    switch (_msg.msg_state)
    {
        ...
        case FD1UART_msg_parser::FD1UART_PREAMBLE2:
            if (temp == PREAMBLE2)
            {
                ...
                _msg.read++;
                _msg.sum += temp;
                _msg.length = FD1_MSG_LS_STATUS_LEN;
...
void FD1_msg_LS_status::process_message(void)
{
    int16_t i = 0;
...
void FD1_msg_LS_status::make_sum()
{
    _msg_1.content.msg.header.head_1 = FD1_msg_LS_status::PREAMBLE1;
    _msg_1.content.msg.header.head_2 = FD1_msg_LS_status::PREAMBLE2;
    _msg_1.content.msg.type = 0xA1;
    _msg_1.content.msg.length = FD1_MSG_LS_STATUS_LEN;
