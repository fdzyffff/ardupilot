...
void AP_MotorsBi::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    // set update rate for the 2 motors (but not the servo on channel 3 or 4)
    ...
    // find the servos
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_motor3, AP_MOTORS_CH_BI_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "MotorsBi: unable to setup servo channel");
        // don't set initialised_ok
        return;
    }
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_motor4, AP_MOTORS_CH_BI_2)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "MotorsBi: unable to setup servo channel");
        // don't set initialised_ok
        return;
    }
    ...
    SRV_Channels::set_angle(SRV_Channels::get_motor_function(AP_MOTORS_CH_BI_1), _yaw_servo_angle_max_deg*100);
    ...
    set_initialised_ok(frame_class == MOTOR_FRAME_BI);
}
...
void AP_MotorsBi::set_frame_class_and_type(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    set_initialised_ok((frame_class == MOTOR_FRAME_BI) && SRV_Channels::function_assigned(SRV_Channel::k_motor3) && SRV_Channels::function_assigned(SRV_Channel::k_motor4));
}
...
void AP_MotorsBi::output_to_motors()
{
    switch (_spool_state) {
        ...
        case SpoolState::SPOOLING_DOWN:
            ...
            rc_write(AP_MOTORS_MOT_1, output_to_pwm(_actuator[1]));
            rc_write(AP_MOTORS_MOT_2, output_to_pwm(_actuator[2]));
            rc_write_angle(AP_MOTORS_CH_BI_1, degrees(_servo_angle_right)*100);
            rc_write_angle(AP_MOTORS_CH_BI_2, degrees(_servo_angle_left)*100);
            break;
    }
}
...
uint32_t AP_MotorsBi::get_motor_mask()
{
...
void AP_MotorsBi::output_armed_stabilizing()
{
    ...
    _yaw_servo_angle_max_deg.set(constrain_float(_yaw_servo_angle_max_deg, AP_MOTORS_BI_SERVO_RANGE_DEG_MIN, AP_MOTORS_BI_SERVO_RANGE_DEG_MAX));
    ...
    if (fabsf(_pivot_yaw) > radians(yaw_max_angle)) {
        limit.yaw = true;
        _pivot_yaw = constrain_float(_pivot_yaw, -radians(yaw_max_angle), radians(yaw_max_angle));
    }
...
void AP_MotorsBi::_output_test_seq(uint8_t motor_seq, int16_t pwm)
{
 ...
 */
void AP_MotorsBi::output_motor_mask(float thrust, uint8_t mask, float rudder_dt)
{
...
float AP_MotorsBi::get_roll_factor(uint8_t i)
{
    float ret = 0.0f;
