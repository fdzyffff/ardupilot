...
void AP_MotorsSkateboard::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    // find the yaw servo
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_engine_srv_0, CH_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "SRV1: Fail to setup");
        // don't set initialised_ok
        return;
    }
    // find the yaw servo
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_engine_srv_1, CH_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "SRV1: Fail to setup");
        // don't set initialised_ok
        return;
    }
    // find the yaw servo
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_engine_srv_2, CH_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "SRV1: Fail to setup");
        // don't set initialised_ok
        return;
    }
    // find the yaw servo
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_engine_srv_3, CH_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "SRV1: Fail to setup");
        // don't set initialised_ok
        return;
    }
    // find the yaw servo
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_engine_srv_4, CH_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "SRV1: Fail to setup");
        // don't set initialised_ok
        return;
    }
    // find the yaw servo
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_engine_srv_5, CH_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "SRV1: Fail to setup");
        // don't set initialised_ok
        return;
    }
    // find the yaw servo
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_engine_srv_6, CH_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "SRV1: Fail to setup");
        // don't set initialised_ok
        return;
    }
    // find the yaw servo
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_engine_srv_7, CH_1)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "SRV1: Fail to setup");
        // don't set initialised_ok
        return;
    }
