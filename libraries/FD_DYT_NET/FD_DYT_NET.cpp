...
uint32_t FD_DYT_NET::port_avaliable(void) {
    if(!initialized()) {
        return false;
    }
...
void FD_DYT_NET::read(uint8_t temp)
{    
    if(!initialized()) {
        return ;
    }
...
void FD_DYT_NET::write(uint8_t temp)
{
    if(!initialized()) {
        return ;
    }
