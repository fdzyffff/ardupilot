...
const AP_Param::GroupInfo AS_RollController::var_info[] = {
    // @Param: 2SRV_TCONST
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("2SRV_TCONST",      0, AS_RollController, gains.tau,       0.5f),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("2SRV_RMAX",   4, AS_RollController, gains.rmax_pos,       0),
    ...
    AP_SUBGROUPINFO(rate_pid, "_RATE_", 9, AS_RollController, AC_PID),

    AP_GROUPEND
};

// constructor
AS_RollController::AS_RollController(const AP_Vehicle::FixedWing &parms)
    : aparm(parms)
{
...
*/
float AS_RollController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, bool ground_mode)
{
...
*/
float AS_RollController::get_rate_out(float desired_rate, float scaler)
{
...
*/
float AS_RollController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode)
{
    if (gains.tau < 0.05f) {
        gains.tau.set(0.05f);
    }
...
void AS_RollController::reset_I()
{
    _pid_info.I = 0;
    rate_pid.reset_I();
}
 ...
 */
void AS_RollController::convert_pid()
{
...
/*
  start an autotune
 */
void AS_RollController::autotune_start(void)
{
    if (autotune == nullptr) {
 ...
 */
void AS_RollController::autotune_restore(void)
{
    if (autotune != nullptr) {
        autotune->stop();
    }
