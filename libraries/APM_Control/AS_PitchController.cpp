...
const AP_Param::GroupInfo AS_PitchController::var_info[] = {
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("2SRV_TCONST",      0, AS_PitchController, gains.tau,       0.5f),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("2SRV_RMAX_UP",     4, AS_PitchController, gains.rmax_pos,   0.0f),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("2SRV_RMAX_DN",     5, AS_PitchController, gains.rmax_neg,   0.0f),
    ...
    // @Increment: 0.05
    // @User: Standard
    AP_GROUPINFO("2SRV_RLL",      6, AS_PitchController, _roll_ff,        1.0f),
    ...
    AP_SUBGROUPINFO(rate_pid, "_RATE_", 11, AS_PitchController, AC_PID),

    AP_GROUPEND
};

AS_PitchController::AS_PitchController(const AP_Vehicle::FixedWing &parms)
    : aparm(parms)
{
...
*/
float AS_PitchController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, float aspeed, bool ground_mode)
{
...
*/
float AS_PitchController::get_rate_out(float desired_rate, float scaler)
{
    float aspeed;
 ...
 */
float AS_PitchController::_get_coordination_rate_offset(float &aspeed, bool &inverted) const
{
    float rate_offset;
...
// 5) maximum FBW airspeed (metres/sec)
//
float AS_PitchController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode)
{
...
void AS_PitchController::reset_I()
{
    _pid_info.I = 0;
    rate_pid.reset_I();
}
 ...
 */
void AS_PitchController::convert_pid()
{
...
/*
  start an autotune
 */
void AS_PitchController::autotune_start(void)
{
    if (autotune == nullptr) {
 ...
 */
void AS_PitchController::autotune_restore(void)
{
    if (autotune != nullptr) {
        autotune->stop();
    }
