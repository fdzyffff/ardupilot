...
// Convenience macros //////////////////////////////////////////////////////////
//
const AP_Param::GroupInfo FD_Target_K230::var_info[] = {

    AP_GROUPINFO("TOUT",   0, FD_Target_K230, target_timeout,        2000),
    AP_GROUPINFO("PIX_W",  1, FD_Target_K230, cam_width,             360),
    AP_GROUPINFO("PIX_H",  2, FD_Target_K230, cam_height,            360),
    AP_GROUPINFO("ANG_X",  3, FD_Target_K230, cam_angle_x,           60.0f),
    AP_GROUPINFO("ANG_Y",  4, FD_Target_K230, cam_angle_y,           60.0f),

    AP_GROUPEND
};

FD_Target_K230::FD_Target_K230()
{
    AP_Param::setup_object_defaults(this, var_info);
    return;
}

bool FD_Target_K230::init() {
    _last_ms = 0;
    _valid = false;
...
void FD_Target_K230::update() {
    ...
    if (tmp_msg._msg_1.updated) {
        ...
        if (tmp_msg._msg_1.content.msg.tag_ok) {
            _last_ms = millis();
            float p1 = -cal_frame_angle(cam_width.get(), cam_angle_x.get(), tmp_msg._msg_1.content.msg.tag_x); // x-axis, degree
            float p2 =  cal_frame_angle(cam_height.get(), cam_angle_y.get(), tmp_msg._msg_1.content.msg.tag_y); // y-axis, degree
...
float FD_Target_K230::cal_frame_angle(float pixel, float angle, float x_in)
{
    // pixel, eg: 1080
    // angle, eg: 54°
    // x_in, eg: 540
...
void FD_Target_K230::handle_info_test(float p1, float p2) {
    handle_info(p1, p2);
...
void FD_Target_K230::handle_msg(const mavlink_message_t &msg)
{
