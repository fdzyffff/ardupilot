...
void FD_Target_RK3588::update() {
    ...
    if (tmp_msg._msg_1.updated) {
        ...
        if (tmp_msg._msg_1.content.msg.tag_cl > 0.5f) {
            _last_ms = millis();
            float p1 = -cal_frame_angle(cam_angle_x.get(), tmp_msg._msg_1.content.msg.tag_x); // x-axis, degree
            float p2 =  cal_frame_angle(cam_angle_y.get(), tmp_msg._msg_1.content.msg.tag_y); // y-axis, degree
