...
AP_ExternalAHRS_ESIM::AP_ExternalAHRS_ESIM(AP_ExternalAHRS *_frontend,
        AP_ExternalAHRS::state_t &_state): AP_ExternalAHRS_backend(_frontend, _state)
{
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AP_ExternalAHRS_ESIM::update_thread, void), "ESITL", 2048, AP_HAL::Scheduler::PRIORITY_SPI, 0)) {
...
void AP_ExternalAHRS_ESIM::update_thread(void)
{
    hal.scheduler->delay(5000);
    if (uart_ins) {
        if (!port_open_ins) {
...
void AP_ExternalAHRS_ESIM::build_packet_ins()
{
    ...
    while (uart_ins->available() > 0) {
        ...
        if (ret >= MAVLINK_FRAMING_OK) {
            ...
            if (msg.msgid == MAVLINK_MSG_ID_SIM_STATE) {
                mavlink_msg_sim_state_decode(&msg, &sim_state_packet);
                handle_imu();
                handle_ahrs();
            }
        }
...
void AP_ExternalAHRS_ESIM::handle_imu()
{
    last_ins_pkt = AP_HAL::millis();

    imu_data.accel = Vector3f(sim_state_packet.xacc,sim_state_packet.yacc,sim_state_packet.zacc);
                                    // m/s^2

    imu_data.gyro = Vector3f(radians(sim_state_packet.xgyro),
                             radians(sim_state_packet.ygyro),
                             radians(sim_state_packet.zgyro));
                             // rad/s
...
void AP_ExternalAHRS_ESIM::post_gps()
{
    {
...
void AP_ExternalAHRS_ESIM::handle_ahrs()
{
    {
        ...
        // state.quat = QuaternionF(sim_state_packet.q1, sim_state_packet.q2, sim_state_packet.q3, sim_state_packet.q4);
        state.quat.from_euler(sim_state_packet.roll, sim_state_packet.pitch, sim_state_packet.yaw);
        state.location.lng = sim_state_packet.lon_int;
        state.location.lat = sim_state_packet.lat_int;
        state.location.set_alt_cm(sim_state_packet.alt*100.f, Location::AltFrame::ABSOLUTE);
        state.velocity = Vector3f(sim_state_packet.vn,sim_state_packet.ve,sim_state_packet.vd);
        ...
        if (!frontend.has_sensor(AP_ExternalAHRS::AvailableSensor::GPS)) {
            ...
            gps_data.gps_yaw                     = (degrees(sim_state_packet.yaw));
...
void AP_ExternalAHRS_ESIM::handle_airspeed()
{
...
void AP_ExternalAHRS_ESIM::post_airspeed()
{
    {
...
// Get model/type name
const char* AP_ExternalAHRS_ESIM::get_name() const
{
    return "ESIM";
}

bool AP_ExternalAHRS_ESIM::healthy(void) const
{
    uint32_t now = AP_HAL::millis();
    return (now - last_ahrs_pkt < 140);
}

bool AP_ExternalAHRS_ESIM::initialised(void) const
{
    return last_ins_pkt != 0;
}

bool AP_ExternalAHRS_ESIM::pre_arm_check(char *failure_msg, uint8_t failure_msg_len) const
{
    if (!healthy()) {
        hal.util->snprintf(failure_msg, failure_msg_len, "ESIM unhealthy");
        return false;
    }
...
void AP_ExternalAHRS_ESIM::get_filter_status(nav_filter_status &status) const
{
    uint32_t now = AP_HAL::millis();
...
void AP_ExternalAHRS_ESIM::send_status_report(GCS_MAVLINK &link) const
{
    // prepare flags
    uint16_t flags = 0;
...
void AP_ExternalAHRS_ESIM::send_packet_servo()
{
    ...
    if ((enabled_mask & 0xFFFF) != 0) {
        UNUSED_RESULT(mavlink_msg_servo_output_raw_pack(
                1,
                0,
                &msg, 
                AP_HAL::micros(),
...
void AP_ExternalAHRS_ESIM::send_mav_message(mavlink_message_t *msg)
{
    if (!port_open_ins) {return;}
    ...
    uart_ins->write((uint8_t *)_MAV_PAYLOAD(msg), msg->len);
    uart_ins->write((uint8_t *)ck, 2);
}
