...
{    
    ...
    if (_msg_trans.enable())   {_msg_trans.parse(temp);}
    if (_msg_ground.enable())   {_msg_ground.parse(temp);}
}
...
void FD1_UART::write(void)
{
    ...
    if (_msg_trans._msg_1.need_send)
    {
        _msg_trans.swap_message();
        _port->write(_msg_trans._msg_1.content.data, sizeof(_msg_trans._msg_1.content.data));
        //_msg_trans._msg_1.updated = false;
        _msg_trans._msg_1.need_send = false;
        _msg_trans.swap_message();
    }
    if (_msg_ground._msg_1.need_send)
    {
        _msg_ground.swap_message();
        _port->write(_msg_ground._msg_1.content.data, sizeof(_msg_ground._msg_1.content.data));
        //_msg_ground._msg_1.updated = false;
        _msg_ground._msg_1.need_send = false;
        _msg_ground.swap_message();
    }
}
