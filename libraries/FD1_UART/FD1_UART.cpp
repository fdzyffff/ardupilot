...
{    
    ...
    while (_port->available() > 0) {
        ...
        if (_msg_hil_in.enable())   {_msg_hil_in.parse(temp);}
    }
}
...
void FD1_UART::write(void)
{
    ...
    int16_t i = 0;
    if (_msg_hil_out._msg_1.need_send)
    {
        _msg_hil_out.swap_message();
        for(i = 0;i < _msg_hil_out._msg_1.length ; i ++) {
            _port->write(_msg_hil_out._msg_1.content.data[i]);
        }
