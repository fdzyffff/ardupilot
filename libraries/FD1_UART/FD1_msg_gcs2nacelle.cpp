...
FD1_msg_gcs2nacelle::FD1_msg_gcs2nacelle(void)
{
    _enable = false;
...
void FD1_msg_gcs2nacelle::parse(uint8_t temp)
{
...
void FD1_msg_gcs2nacelle::reset(uint8_t temp)
{
    _msg.msg_state = FD1UART_msg_parser::FD1UART_PREAMBLE1;
    parse(temp);
}

void FD1_msg_gcs2nacelle::process_message(void)
{
    _msg.count++;
...
void FD1_msg_gcs2nacelle::swap_message(void)
{
    ;
}
