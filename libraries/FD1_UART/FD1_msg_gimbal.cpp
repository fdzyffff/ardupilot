#include "FD1_msg_gimbal.h"
// #include <GCS_MAVLink/GCS.h>

FD1_msg_gimbal::FD1_msg_gimbal(void)
{
    _enable = false;
...
void FD1_msg_gimbal::parse(uint8_t temp)
{
    switch (_msg.msg_state)
    {
        ...
        case FD1UART_msg_parser::FD1UART_INFO:
            ...
            _msg.read++;
            _msg.sum_check ^= temp;
            if (4 <= _msg.length && _msg.length <= FD1_MSG_GIMBAL_LEN-3) {
                _msg.msg_state = FD1UART_msg_parser::FD1UART_DATA;
            } else {
                _msg.msg_state = FD1UART_msg_parser::FD1UART_PREAMBLE1;
            }
...
void FD1_msg_gimbal::process_message(void)
{
    int16_t i = 0;
...
bool FD1_msg_gimbal::set_id(uint8_t id)
{
    _msg_1.length = 0;
    switch (id) {
        case 0x30:
...
bool FD1_msg_gimbal::sum_check(void)
{
    int16_t i = 0;
    uint8_t sum_check = 0;
    _msg_1.length = _msg_1.content.msg.header.info&0b00111111;

    if (_msg_1.length < 4 || _msg_1.length>FD1_MSG_GIMBAL_LEN-3) {return false;}
...
void FD1_msg_gimbal::swap_message(void)
{
