...
uint32_t FD_DYT_NEW::port_avaliable(void) {
    if(!initialized()) {
        return false;
    }
...
void FD_DYT_NEW::read(uint8_t temp)
{    
    if(!initialized()) {
        return ;
    }
...
void FD_DYT_NEW::write(uint8_t temp)
{
    if(!initialized()) {
        return ;
    }
